...
class CurrenciesConverter extends Component {
  ...
  onOptionChangeHandler = (event) => {
    ...
    const changeValue = parseFloat(
      document.getElementById("pln-change-value").value
    );
  ...
  onInputChangeHandler = () => {
    const changeValue = parseFloat(
      document.getElementById("currency-change-value").value
    );
  ...
  flagRender() {
    if (this.curCurrencyCode !== undefined && this.curCurrencyCode !== "") {
      return (
        <img
          src={require(`../../icons/${this.curCurrencyCode}.svg`)}
          alt={this.curCurrencyCode}
          height={"30px"}
        />
      );
    }
